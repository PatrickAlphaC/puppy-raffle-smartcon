...
contract PuppyRaffle is ERC721, Ownable {
    ...
    /// @notice there must be at least 4 players (different to address(0)), and the duration has occurred
    /// @notice the previous winner is stored in the previousWinner variable
    /// @dev we sanitize the players array in case of some player has called the refund() function
    ...
    function selectWinner() external {
        ...
        // count non zero addresses (the number players)
        uint256 nonZero;
        ...
        require(nonZero >= 4, "PuppyRaffle: Need at least 4 players");

        // go through the players array to eliminate the zero addresses
        // we create a another array not with the length of players
        // because it may contain a null address which we don't want
        address[] memory sanitizedPlayers = new address[](nonZero);
        uint256 j;
        for (uint256 i = 0; i < players.length; i++) {
            if (players[i] != address(0)) {
                sanitizedPlayers[j] = players[i];
                j++;
            }
        }
        ...
        address winner = sanitizedPlayers[winnerIndex];
        uint256 totalAmountCollected = sanitizedPlayers.length * entranceFee;
